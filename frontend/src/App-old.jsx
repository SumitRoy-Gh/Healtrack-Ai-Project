...
// motion is used in VisualAnalysis, AIReports, and UploadModal components
import {
  AreaChart,
  Area,
  XAxis,
...
const Header = ({ onUploadClick }) => {
  ...
  const handleShare = () => {
    // Robust copy to clipboard (includes iframe fallback)
    ...
    <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between pb-6 mb-6 border-b border-slate-200 gap-4">
      <div>
        ...
        <p className="text-sm text-slate-500 font-medium">
          Patient:{" "}
          <span className="text-slate-900">{MOCK_DATA.patient_id}</span> • Day{" "}
          {MOCK_DATA.current_day} of Assessment
        </p>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-lg font-semibold text-rose-600 mt-1">
            {metrics.contributing_factors[0]}
          </div>
...
const VisualAnalysis = () => {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
      ...
      {/* Right Column: Simulation (The Wow Factor) */}
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-white border border-slate-200 p-3 rounded-lg shadow-lg text-sm">
        <p className="font-semibold text-slate-900 mb-1">Day {label}</p>
...
const DynamicAnalytics = () => {
  const { chartData, metrics } = MOCK_DATA;
  const gaugeData = [
          ...
          {activeTab === "patient" && (
            ...
            >
              <ul className="space-y-3">
                {ai_report.patient_advice.map((advice, index) => (
                  <li
                    key={index}
      ...
      >
        ...
        <div className="p-6 space-y-4 overflow-y-auto">
          ...
          <div className="space-y-2">
            ...
            <textarea
              id="notes"
              className="w-full h-24 p-3 text-sm border border-slate-200 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent resize-none"
              placeholder="e.g., Patient reported mild pain during dressing change, slight fever yesterday."
            />
          </div>
        </div>
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-2 shrink-0">
          ...
          <Button onClick={onClose} disabled={!file}>
            {file ? "Analyze Scan" : "Select a File"}
          </Button>
        </div>
      </motion.div>
    </div>
